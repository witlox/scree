...
def _client() -> TestClient:
    return TestClient(create_app(DocStore([]), Authority({}), allow_insecure_header_auth=True))
...
def test_assess_returns_derived_fields_and_category_trigger():
    ...
    delivery = client.post("/risks/assess", json={"category": "delivery", "likelihood": 5, "impact": 4}, headers=AUTH).json()
    ...
    security = client.post("/risks/assess", json={"category": "security", "likelihood": 1, "impact": 1}, headers=AUTH).json()
